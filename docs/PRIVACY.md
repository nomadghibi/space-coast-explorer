...
Raw GPS readings are processed locally for stop proximity. M2 stores tour progress in localStorage, but not continuous movement history. Future analytics may record events such as `tour_start`, `stop_arrival`, `stop_complete`, and `tour_complete`; they must not include second-by-second location trails.
